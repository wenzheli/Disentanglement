...
% 
addpath('../analysis');
dimZs = [2 3]; % number of nodes per block
...
for paramIter1 = 1:2
    for paramIter2 = 10:10  % 12 possible learning parameters
        ...
        elseif(strcmp(NN.algInit, 'ptZ'))
            ...
            NN = pretrainH1(NN, NNsetting);
            NN = pretrainC_fullZ(NN, NNsetting, dataTr);
        ...
        for epoch = 1:NNsetting.nEpoch
            ...
            for batchIdx = 1:numBatch
                ...
                NN = fullFF(NN, NNsetting, dataTr(:,index(firstIdx:lastIdx)));
                ...
                NN = fullBackProp(NN, NNsetting, NW);
            ...
            if(epoch==1 || epoch==20)
                if(strcmp(alg, 'sgd'))
                    nameNN = ['modelC_dim' num2str(NN.D2) 'epoch' num2str(epoch) 'SGD' num2str(learnParam) 'init' algInit '.mat'];
                elseif(strcmp(alg, 'adadelta'))
                    nameNN = ['modelC_dim' num2str(NN.D2) 'epoch' num2str(epoch) 'Ada' num2str(learnParam) 'init' algInit '.mat'];
                end
                NN.prior = NW;
                saveModelCfull(nameNN, NN);
            end
        end
    end
end
