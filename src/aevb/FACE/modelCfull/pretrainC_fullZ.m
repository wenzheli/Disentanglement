function [NN] = pretrainC_fullZ(NN, NNsetting, dataTr)
%
    NW = defaultNWinit(NN);
    ...
    for epoch = 1:NNsetting.nEpoch
        ...
        for batchIdx = 1:numBatch
            ...
            NN = ptC_ffFullZ(NN, NNsetting, reconVar);
            ...
            NN = ptC_backFullZ(NN, NNsetting, NW);
            
            %% update NN parameter
            NN = ptC_updateFullZ(NN, NNsetting);
            
        end
    end
end

function NN = ptC_ffFullZ(NN, setting, x)
    L = setting.L;
...
function NN = ptC_backFullZ(NN, setting, nw)
    L = setting.L;
    mbSize = setting.mbSize;
...
function NN = ptC_updateFullZ(NN, setting)
