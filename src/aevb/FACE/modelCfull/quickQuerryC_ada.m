...
names = dir('modelC_*dim2*epoch20*Ada*random*mat');
if(~isempty(names))
...
names = dir('modelC_*dim3*epoch20*Ada*random*mat');
if(~isempty(names))
    nModels = length(names);
    for i=1:nModels
        load(names(i).name);
...
names = dir('modelC_*dim4*epoch20*Ada*random*mat');
if(~isempty(names))
    nModels = length(names);
    for i=1:nModels
        load(names(i).name);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%5
% ptZ, dimension = 2*3
names = dir('modelC_*dim2*epoch20*Ada*ptZ*mat');
if(~isempty(names))
    nModels = length(names);
    for i=1:nModels
        load(names(i).name);
...
% ptZ, dimension = 3*3
names = dir('modelC_*dim3*epoch20*Ada*ptZ*mat');
if(~isempty(names))
    nModels = length(names);
    for i=1:nModels
        load(names(i).name);
...
% ptZ, dimension = 4*3
names = dir('modelC_*dim4*epoch20*Ada*ptZ*mat');
if(~isempty(names))
    nModels = length(names);
    for i=1:nModels
        load(names(i).name);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%5
% ptH, dimension = 2*3
names = dir('modelC_*dim2*epoch20*Ada*ptH*mat');
if(~isempty(names))
    nModels = length(names);
    for i=1:nModels
        load(names(i).name);
...
% ptH, dimension = 3*3
names = dir('modelC_*dim3*epoch20*Ada*ptH*mat');
if(~isempty(names))
    nModels = length(names);
    for i=1:nModels
        load(names(i).name);
...
% ptH, dimension = 4*3
names = dir('modelC_*dim4*epoch20*Ada*ptH*mat');
if(~isempty(names))
    nModels = length(names);
    for i=1:nModels
        load(names(i).name);
