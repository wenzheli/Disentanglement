...
%       {delta1, deltaMu, deltaAT, deltaAM, deltaBeta} --> delta0
    ...
    for m=1:NN.nClasses
        NN.delta1{m} = NN.delta1{m} + NN.Wc{m}*NN.deltaC{m};
    end
    ...
    for classID = 1:NN.nClasses
        NN.dWc{classID} = NN.Z{NN.mapBlock(classID)}*NN.deltaC{classID}'/mbSize/L + 0.5*NN.dWc{classID};
        NN.dbc{classID} = sum(NN.deltaC{classID}, 2)/mbSize/L  + 0.5*NN.dbc{classID};
    end
