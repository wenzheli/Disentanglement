    ...
    for epoch = 1:nEpoch
        ...
        for batchIdx = 1:numBatch
            if(earlyStop==0)
                ...
                energy = energy*0;
                energy(1) = sum(sum((X2-Y).^2));

                for blockID = 1:nBlocks
                    for i=1:mbSize
                        Sigma{blockID}(:,:,i) = AT{blockID}(:,:,i)*AT{blockID}(:,:,i)';
%                         energy(2) = energy(2) + 0.5*log(det(Sigma{blockID}(:,:,i))+1e-16);
%                         energy(3) = energy(3) - 0.5*trace(Wnu{blockID}*WW{blockID}*Sigma{blockID}(:,:,i));
%                         energy(4) = energy(4) - ...
%                             0.5*(eta{blockID}(:,i) - Wmu{blockID})'*Wnu{blockID}*WW{blockID}*(eta{blockID}(:,i)-Wmu{blockID});
                    end
%                     energy(5) = -0.5*log(det(WW{blockID}));
%                     for d=1:D2
%                         energy(5) = energy(5) - 0.5*psi((Wnu{blockID}-D2+d)/2);
%                     end
%                     energy(5) = energy(5)+0.5*D2/Wbeta{blockID};
%                     energy(5) = energy(5)*mbSize;
                end

                LL(epoch,:) = LL(epoch,:) + energy;
                hist.E = [hist.E; LL(epoch,:)/mbSize/batchIdx];
                if(epoch>=8 && LL(epoch, 1)/mbSize/batchIdx/L <= -205)
                    earlyStop =1;
                    continue
                end
                ...
                if(rem(batchIdx,400)==0)
                    fprintf('epoch %d, batch %d, energy %f, %f, %f, %f, %f, %f\n', ...
                        epoch, batchIdx, LL(epoch, 1)/mbSize/batchIdx/L, ...
                        LL(epoch, 2)/mbSize/batchIdx, LL(epoch, 3)/mbSize/batchIdx, ...
                        LL(epoch, 4)/mbSize/batchIdx, LL(epoch, 5)/mbSize/batchIdx, ...
                        sum(LL(epoch,:))/mbSize/batchIdx);
                end

                %% backpropagation - 1: w.r.t. layers
                delta3 = X2-Y;
                delta2 = (W5*delta3).*h2.*(1-h2);
...
%                             Wnu{blockID}*WW{blockID}*(eta{blockID}(:,i)-Wmu{blockID});
    ...
    models{hid}.LL(:,1) = LL(:,1)/nSamples/L;
    models{hid}.LL(:,2:4) = LL(:,2:4)/nSamples;
    models{hid}.energy = hist.E(2:end,:);

    save(['aevbBlock' num2str(nBlocks) 'NWPriorDim' num2str(D2) '.mat'],'models');
end
