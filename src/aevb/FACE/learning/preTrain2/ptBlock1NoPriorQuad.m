        ...
        for batchIdx = 1:numBatch
            if(earlyStop==0)
                ...
                for blockID = 1:nBlocks
                    ...
                    AM{blockID} = bsxfun(@plus, W2{blockID}'*X1, b2{blockID});
...
%                 end
                ...
                for blockID = 1:nBlocks
                    ...
                    dW2{blockID} = X1*deltaAM{blockID}'/mbSize;
                    dW1{blockID} = X1*deltaEta{blockID}'/mbSize;
                end

                db3 = sum(deltaH,2)/mbSize/L;
                for blockID = 1:nBlocks
                    db2{blockID} = sum(deltaAM{blockID},2)/mbSize;
...
%     model{hid}.energy = hist.E(2:end);
    ...
    save(['ptNoPriorBlock' num2str(nBlocks) 'Dim' num2str(D1) '.mat'],'model');
end
