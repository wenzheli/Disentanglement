...
% initialize the NN parameters RANDOMLY
    ...
    elseif(strcmp(NN.shape, 'full'))
        ...
        NN.W3 = (rand(NN.D1, NN.D2^2)-0.5)/10;
        NN.b3 = (rand(NN.D2^2,1)-0.5)/10;
        ...
        NN.delta3      = zeros(NN.D0, setting.mbSize*setting.L);
        NN.delta2      = zeros(NN.D3, setting.mbSize*setting.L);
        NN.delta1      = zeros(NN.D2, setting.mbSize*setting.L);
        NN.deltaMu     = zeros(NN.D2, setting.mbSize);
        NN.deltaAT     = zeros(NN.D2, NN.D2, setting.mbSize);
        NN.deltaAM     = zeros(NN.D2^2, setting.mbSize);
        NN.deltaBeta   = zeros(NN.D2^2, setting.mbSize);
        NN.delta0      = zeros(NN.D1, setting.mbSize);
