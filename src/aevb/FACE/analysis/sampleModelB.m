...
% sample the observations
    ...
    [dataTr, labelTr, model.faceM, model.faceSTD] = load_Face_Data();
...
%   size of bias:   [Dimout, 1]
    ...
    for blkid=1:nBlocks
        ...
        idx = find(eye(length(NN.blocks{blkid}))==1);
        NN.W3{blkid} = zeros(size(W3,1), NN.D2^2); 
        NN.b3{blkid} = zeros(NN.D2.^2,1);
        NN.W3{blkid}(:, idx) = W3(:, NN.blocks{blkid});
        NN.b3{blkid}(idx) = b3(NN.blocks{blkid});
...
function [dataTr, labelTr, faceM, faceSTD] = load_Face_Data()
