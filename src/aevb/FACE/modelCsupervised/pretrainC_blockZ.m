        ...
        for batchIdx = 1:numBatch
            ...
            if(rem(batchIdx,100)==1)
                fprintf(2,'epoch %d, minibatch %d, recon loss: %f, %s\n', ...
                    ...
                    num2str(NN.ptZLossPred(epoch,:)/batchIdx/mbSize/NNsetting.L));
            end
...
function NN = ffBlockZ(NN, setting, Y1)
    ...
    for classID=1:NN.nClasses
        ...
        NN.lossPred(classID) = sum(l1{classID}~=l2{classID});
        NN.deltaC{classID} = NN.cWeight(classID)*(NN.deltaC{classID} - NN.predY{classID});
    end
end
