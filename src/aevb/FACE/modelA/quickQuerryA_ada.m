% random, dimension = 2*3
names = dir('modelA_*dim6*epoch20*Ada*random*mat');
if(~isempty(names))
    ...
    fprintf(2,'\ndimension=6, random, minibatch-Sampler\n');
    disp(num2str(losses1));
    [a,b] = min(losses1(:,3));
    disp(names(b).name);
end

% random, dimension = 3*3
names = dir('modelA_*dim9*epoch20*Ada*random*mat');
if(~isempty(names))
    nModels = length(names);
    for i=1:nModels
        load(names(i).name);
    ...
    fprintf(2,'\ndimension=9, random, minibatch-Sampler\n');
    disp(num2str(losses2));
    [a,b] = min(losses2(:,3));
    disp(names(b).name);
end

% random, dimension = 4*3
names = dir('modelA_*dim12*epoch20*Ada*random*mat');
if(~isempty(names))
    nModels = length(names);
    for i=1:nModels
        load(names(i).name);
    ...
    fprintf(2,'\ndimension=12, random, minibatch-Sampler\n');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%5
% ptZ, dimension = 2*3
names = dir('modelA_*dim6*epoch20*Ada*ptZ*mat');
if(~isempty(names))
    nModels = length(names);
    for i=1:nModels
        load(names(i).name);
...
% ptZ, dimension = 3*3
names = dir('modelA_*dim9*epoch20*Ada*ptZ*mat');
if(~isempty(names))
    nModels = length(names);
    for i=1:nModels
        load(names(i).name);
...
% ptZ, dimension = 4*3
names = dir('modelA_*dim12*epoch20*Ada*ptZ*mat');
if(~isempty(names))
    nModels = length(names);
    for i=1:nModels
        load(names(i).name);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%5
% ptH, dimension = 2*3
names = dir('modelA_*dim6*epoch20*Ada*ptH*mat');
if(~isempty(names))
    nModels = length(names);
    for i=1:nModels
        load(names(i).name);
    ...
    fprintf(2,'\ndimension=6, ptH, minibatch-Sampler\n');
...
% ptH, dimension = 3*3
names = dir('modelA_*dim9*epoch20*Ada*ptH*mat');
if(~isempty(names))
    nModels = length(names);
    for i=1:nModels
        load(names(i).name);
    ...
    fprintf(2,'\ndimension=9, ptH, minibatch-Sampler\n');
...
% ptH, dimension = 4*3
names = dir('modelA_*dim12*epoch20*Ada*ptH*mat');
if(~isempty(names))
    nModels = length(names);
    for i=1:nModels
        load(names(i).name);
    ...
    fprintf(2,'\ndimension=12, ptH, minibatch-Sampler\n');
